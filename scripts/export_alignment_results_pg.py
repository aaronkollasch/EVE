...
if tier == 1:
    ...
    assert (prio_df["protein_index"] != -1).all(), prio_df[prio_df["protein_index"] == -1]
...
def make_a2ms(run_name, project_name, tup):
    ...
    while run < max_runs:
        try:
            subprocess.run(["aws", "s3", "cp", f"s3://markslab-us-east-2/colabfold/output/{project_name}/MSA/{run_name}/{i}.a3m", f"/tmp/{run_name}/{msa_location}.a3m"], check=True)
        except subprocess.CalledProcessError as e:
            error = e
            continue
        else:
    ...
    while run < max_runs:
        try:
            subprocess.run(["aws", "s3", "cp", f"/tmp/{run_name}/{protein}.a2m", f"s3://markslab-private/eve/{project_name}/data/MSA/{run_name}/{msa_location}.a2m"], check=True)
        except subprocess.CalledProcessError as e:
            error = e
            continue
        else:
...
for run_name in run_names:
    ...
    def get_run_files():
        for row in prio_df.itertuples():
            yield row.protein_index, row.protein
